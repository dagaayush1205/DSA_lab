...
class DFSVisualizer:
    def __init__(self, root):
        self.root = root
        self.root.title("Depth-First Search (DFS) Visualizer - Directed Graph")
        self.graph = {}
        ...
        self.info_label = tk.Label(self.root, text="Click on canvas to add nodes and directed edges.")
    ...
    def add_edges(self, node_name):
        for other_node in self.graph:
            if other_node != node_name:
                if messagebox.askyesno("Add Edge", f"Add directed edge from {node_name} to {other_node}?"):
                    # Add directed edge from node_name to other_node only
                    self.graph[node_name].append(other_node)
                    
                    x1, y1 = self.nodes[node_name]
                    x2, y2 = self.nodes[other_node]
                    
                    # Draw a directed line with an arrow at the end
                    self.edges.append(self.canvas.create_line(x1, y1, x2, y2, fill="black", arrow=tk.LAST))
